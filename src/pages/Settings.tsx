  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      <div className="max-w-[560px] mx-auto px-5 py-6 md:py-10">
        ...
        <p className="mt-1 text-xs text-ink-3">账号与登录安全</p>

        {/* 当前账号 */}
        <div className="mt-6 rounded-xl border border-line bg-white p-5">
          <div className="text-xs font-medium text-ink-2 mb-2">当前登录账号</div>
          <div className="text-sm text-ink">{email || '—'}</div>
        </div>

        {authEnabled ? (
          <form onSubmit={submit} className="mt-4 rounded-xl border border-line bg-white p-5">
        ...
        {/* 保险箱口令（敏感字段加密口令） */}
        {isCloud ? (
          <form onSubmit={changeVault} className="mt-4 rounded-xl border border-line bg-white p-5">
